...
static void tx_buffer_process(void)
{
    if (m_tx_index != m_tx_insert_index)
    {
        ...
        {
            printf("[LBS_C]: SD Read/Write API returns Success..\r\n");
            m_tx_index++;
            m_tx_index &= TX_BUFFER_MASK;
        }
        else
        {
            printf("[LBS_C]: SD Read/Write API returns error. This message sending will be "
                "attempted again..\r\n");
        }
    }
}
...
void ble_lbs_on_db_disc_evt(ble_lbs_c_t * p_ble_lbs_c, const ble_db_discovery_evt_t * p_evt)
{
        ...
        p_evt->params.discovered_db.srv_uuid.type == p_ble_lbs_c->uuid_type)
    {
        ...
        printf("[LBS_C]: Led Button Service discovered at peer.\r\n");
...
{
    printf("[LBS_C]: Configuring CCCD. CCCD Handle = %d, Connection Handle = %d\r\n",
...
uint32_t ble_lbs_led_status_send(ble_lbs_c_t * p_ble_lbs_c, uint8_t status)
{
    ...
    printf("[LBS_C]: writing LED status 0x%x", status);
